...
{
    public abstract class CR_StriderUpgrade : CR_StriderBase
    {
        [SerializeField]
        protected bool m_DrawGUIStats;
        protected IF_Stats m_GUIStats;
        ...
        public override void Initialize()
        {
            ...
            m_GUIStats = GetComponent<IF_Stats>();

            if (m_DrawGUIStats && m_GUIStats == null)
            {
                m_GUIStats = gameObject.AddComponent<IF_Stats>();
            }
        }
        ...
        {
            m_GUIStats.Add(TargetSpeed, "Speed", "Target", UT_ChasisColors.Orange);
            m_GUIStats.Add(m_Body.AvgSpeed, "Speed", "Measured Avg.", UT_ChasisColors.Lightblue);

            m_GUIStats.Add(m_Body.TargetHeight, "Height", "Target", UT_ChasisColors.Orange);
            m_GUIStats.Add(m_Body.AvgHeight, "Height", "Measured Avg.", UT_ChasisColors.Lightblue);

            m_GUIStats.Add(m_Body.Inclination / 180f, "Inclination", "", UT_ChasisColors.Orange);

            m_GUIStats.Add(GetNormWalkDirectionError(), "Direction Errors", "Walk Direction", UT_ChasisColors.Orange);
            m_GUIStats.Add(GetNormLookDirectionError(), "Direction Errors", "Look Direction", UT_ChasisColors.Lightblue);

            var palette = UT_ChasisColors.Palette(4, 1, 0.5f, 0.2f, 0.8f);
            ...
            sum += m_GUIStats.Add(GetSpeedErrorPenalty(), "Penalties", "Speed Error", UT_ChasisColors.Orange);

            m_GUIStats.Add(sum, "Reward Sum", "", UT_ChasisColors.Lightblue);
        }
    }
}