...
{
    public class IF_GraphCore : MonoBehaviour
    {
        public class Line
        {
            ...
            public UT_StatsBuffer Buffer { get; private set; }
            public bool HasLabel => Buffer.Name.Length > 0;
            public bool IsDrawable => Buffer.IsEnabled && Buffer.Length > 1;

            public Line(UT_StatsBuffer buffer, Color color, bool showValue)
            {
                Buffer = buffer;
                Color = color;
        ...
        public IF_GraphCore Add(UT_StatsBuffer data, Color color, bool showValue = false)
        {
